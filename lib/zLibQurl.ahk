...
class LibQurl {
    ...
    Init(){
        ...
        this.handleMap[easy_handle] := this.handleMap[0] := Map() ;handleMap[0] is a dynamic reference to the last created easy_handle
        If !this.handleMap[easy_handle]
            throw ValueError("Problem in 'curl_easy_init'! Unable to init easy interface!", -1, this.curlDLLpath)
        this.handleMap[easy_handle]["easy_handle"] := easy_handle
        this.handleMap[easy_handle]["options"] := Map()  ;prepares option storage
        ...
        this.handleMap[easy_handle]["callbacks"] := Map()  ;prepares write callbacks
        for k,v in ["body","header","read","progress","debug"]{
            this.handleMap[easy_handle]["callbacks"][v] := Map()
            this.handleMap[easy_handle]["callbacks"][v]["CBF"] := ""
        }
    ...
    ; DupeInit(easy_handle?){
        ...
        ; this.handleMap[newHandle] := this.handleMap[0] := this.DeepClone(this.handleMap[easy_handle])
        ...
        ; this.handleMap[newHandle] := this.handleMap[0] := Map() ;handleMap[0] is a dynamic reference to the last created easy_handle
        ; ,this.handleMap[newHandle]["options"] := Map()  ;prepares option storage
        ; for k,v in this.handleMap[easy_handle]["options"]
        ;     this.SetOpt(k,v,newHandle)
        ; this.handleMap[newHandle]["easy_handle"] := newHandle
        ; return newHandle 
        /*
        if !IsSet(easy_handle)
            easy_handle := this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        newHandle := this._curl_easy_duphandle(easy_handle)
        If !this.handleMap[easy_handle]
            throw ValueError("Problem in 'curl_easy_init'! Unable to init easy interface!", -1, this.curlDLLpath)
        ; msgbox easy_handle "`n" newHandle "`n`n" this.handleMap[0]["easy_handle"]
        this.handleMap[newHandle] := this.DeepClone(this.handleMap[easy_handle])
        msgbox this.ShowOB(this.handleMap[newHandle])
        this.handleMap[0]["easy_handle"] := this.handleMap[newHandle]["easy_handle"]
        ; msgbox this.handleMap[newHandle]["easy_handle"] "`n" this.handleMap[easy_handle]["easy_handle"] "`n`n" this.handleMap[0]["easy_handle"]
        ; this.handleMap[newHandle] := this.handleMap[0] := Map() ;handleMap[0] is a dynamic reference to the last created easy_handle
        ; ,this.handleMap[newHandle]["options"] := Map()  ;prepares option storage
    ...
    SetOpt(option,parameter,easy_handle?){
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        this.handleMap[easy_handle]["options"][option] := parameter
        return this._curl_easy_setopt(easy_handle,option,parameter)
    }
    ...
    WriteToFile(filename, easy_handle?) {
        ...
        this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"] := LibQurl.Storage.File(filename, &passedHandleMap, "body", "w", easy_handle)
        this.SetOpt("WRITEDATA",this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"],easy_handle)
        this.SetOpt("WRITEFUNCTION",this.handleMap[easy_handle]["callbacks"]["body"]["CBF"],easy_handle) 
        Return
    }
    ...
    WriteToMem(maxCapacity := 0, easy_handle?) {
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        passedHandleMap := this.handleMap
        this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"] := LibQurl.Storage.MemBuffer(dataPtr?, maxCapacity?, dataSize?, &passedHandleMap, "body", easy_handle)
        ; this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"].Ptr := this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"]
        ; writeTo := this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"]:= LibQurl.Storage.MemBuffer(dataPtr?, maxCapacity?, dataSize?, &passedHandleMap, "body", easy_handle)
        
        this.SetOpt("WRITEDATA",this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"].Ptr,easy_handle)
        this.SetOpt("WRITEFUNCTION",this.handleMap[easy_handle]["callbacks"]["body"]["CBF"],easy_handle)
		Return
	}
    ...
    ;     ;instanstiate Storage.File
    ;     passedHandleMap := this.handleMap
    ;     this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"] := class_libcurl.Storage.File(filename, &passedHandleMap, "body", "w", easy_handle)
    ;     this.SetOpt("WRITEDATA",this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"],easy_handle)
    ;     this.SetOpt("WRITEFUNCTION",this.handleMap[easy_handle]["callbacks"]["body"]["CBF"],easy_handle) 
    ;     Return
    ; }
	...
	HeaderToFile(filename, easy_handle?) {
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        passedHandleMap := this.handleMap
        this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"] := LibQurl.Storage.File(filename, &passedHandleMap, "header", "w", easy_handle)
        this.SetOpt("HEADERDATA",this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"],easy_handle)
        this.SetOpt("HEADERFUNCTION",this.handleMap[easy_handle]["callbacks"]["header"]["CBF"],easy_handle)
		Return
	}
	...
	HeaderToMem(maxCapacity := 0, easy_handle?) {
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        passedHandleMap := this.handleMap
		this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"] := LibQurl.Storage.MemBuffer(dataPtr?, maxCapacity?, dataSize?, &passedHandleMap, "header", easy_handle)
        this.SetOpt("HEADERDATA",this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"].writeObj["writeTo"].Ptr,easy_handle)
        this.SetOpt("HEADERFUNCTION",this.handleMap[easy_handle]["callbacks"]["header"]["CBF"],easy_handle)
        Return
	}
    ...
    _setCallbacks(body?,header?,read?,progress?,debug?,easy_handle?){
        ...
        if IsSet(body){
            CBF := this.handleMap[easy_handle]["callbacks"]["body"]["CBF"]
            ...
            this.handleMap[easy_handle]["callbacks"]["body"]["CBF"] := CBF := CallbackCreate(
        ...
        if IsSet(header){
            CBF := this.handleMap[easy_handle]["callbacks"]["header"]["CBF"]
            if IsInteger(CBF){  ;checks if this callback already exists
                this.writeRefs[CBF] -= 1    ;decrement the reference tracker
                ,CallbackFree(CBF)
                ,(CBF=0?writeRefs.delete(CBF):"")   ;remove key if done with it
            }
            ; if IsInteger(this.handleMap[easy_handle]["callbacks"]["header"]["CBF"])
            ;     CallbackFree(this.handleMap[easy_handle]["callbacks"]["header"]["CBF"])
            this.handleMap[easy_handle]["callbacks"]["header"]["CBF"] := CBF := CallbackCreate(
                (dataPtr, size, sizeBytes, userdata) =>
        ...
        ;   }
        ;   this.handleMap[easy_handle]["writeCallbackFunction"] := CallbackCreate(actualCallbackFunction)
    ...
    Perform(easy_handle?){
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"].Open()
        this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"].Open()
        retCode := DllCall("libcurl-x64\curl_easy_perform","Ptr",easy_handle)
        ; msgbox "perform code: " retCode
        this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"].Close()
        this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"].Close()

        ;accessibly attach body to easy_handle output
        bodyObj := this.handleMap[easy_handle]["callbacks"]["body"]
        lastBody := (bodyObj["writeType"]="memory"?bodyObj["writeTo"]:FileOpen(bodyObj["filename"],"rw"))
        this.handleMap[easy_handle]["lastBody"] := lastBody

        ;accessibly attach headers to easy_handle output
        headerObj := this.handleMap[easy_handle]["callbacks"]["header"]
        lastHeaders := (headerObj["writeType"]="memory"?headerObj["writeTo"]:FileOpen(headerObj["filename"],"rw"))
        this.handleMap[easy_handle]["lastHeaders"] := lastHeaders
        return retCode
    }
    GetLastHeaders(returnAsEncoding := "UTF-8",easy_handle?){
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        lastHeaders := this.handleMap[easy_handle]["lastHeaders"]
    ...
    GetLastBody(returnAsEncoding := "UTF-8",easy_handle?){
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        lastBody := this.handleMap[easy_handle]["lastBody"]
    ...
    Cleanup(easy_handle?){
        easy_handle ??= this.handleMap[0]["easy_handle"]   ;defaults to the last created easy_handle
        for k,v in this.handleMap[easy_handle]["callbacks"]
            if IsInteger(this.handleMap[easy_handle]["callbacks"][k]["CBF"])
                CallbackFree(this.handleMap[easy_handle]["callbacks"][k]["CBF"])
        this.handleMap.Delete(easy_handle)
    ...
    _writeCallbackFunction(dataPtr, size, sizeBytes, userdata, easy_handle) {
        ...
        return this.handleMap[easy_handle]["callbacks"]["body"]["storageHandle"].RawWrite(dataPtr, dataSize)
	}

	_headerCallbackFunction(dataPtr, size, sizeBytes, userdata, easy_handle) {
		dataSize := size * sizeBytes
        ; msgbox type(this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"])
		Return this.handleMap[easy_handle]["callbacks"]["header"]["storageHandle"].RawWrite(dataPtr, dataSize)
	}
    ...
    Class Storage {
        ...
        Class File {
            __New(filename, &handleMap, storageCategory, accessMode := "w", easy_handle?) {
                ...
                this.writeObj := this.handleMap[easy_handle]["callbacks"][storageCategory]
            ...
            Open() {
                If (this.writeObj["accessMode"] == "w") {
                    ...
                    ; this.handleMap["assoc"][this.writeObj["writeTo"].easy_handle] := this.getCurlHandle()
            ...
            Close() {
                ...
                ; this.handleMap["assoc"].Delete(this.writeObj["writeTo"].easy_handle)
            }
        ...
        ; Wrapper for memory buffer, similar to regular FileObject
        	...
        	Close() {
        		; this.handleMap[this.easy_handle]["lastHeaders"] := this.writeObj["writeTo"]
    ...
    _curl_easy_setopt(easy_handle, option, parameter, debug?) {
        ;currently specifically crashing on easyType=CURL_CBPTR / type=Ptr
        ;something to do with the MemBuffer
        
        ; if this.opt[option]["type"] = "Ptr"{
        ;     parameter := parameter.writeObj["writeTo"].Ptr
        
        ;     msgbox this.opt[option]["name"] "`n" parameter "`n" Type(parameter)
        ; }
        ; if IsSet(debug)
        ; if this.opt[option]["type"] = "Ptr"
        
        msgbox this.showob(this.opt[option]) "`n`n`n"
            .   "passed easy_handle: " easy_handle "`n"
            .   "passed option id: " this.opt[option]["id"] "`n"
        ...
        retCode := DllCall(this.curlDLLpath "\curl_easy_setopt"
            ...
            ; msgbox this.opt[option]["name"] " works"
        return retCode
    }
